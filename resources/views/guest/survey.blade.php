<x-guest-layout>
    <div class="relative max-w-3xl mx-auto px-4">
        <form method="POST" action="{{ route('guest.survey.store', $visit) }}"
            class="relative space-y-7 rounded-3xl border border-white/20 bg-white/10 backdrop-blur-xl shadow-xl p-6 sm:p-8">
            @csrf

            {{-- Header --}}
            <div class="text-center space-y-2">
                <div
                    class="mx-auto inline-flex h-14 w-14 items-center justify-center rounded-2xl border border-white/20 bg-white/15 backdrop-blur shadow-lg">
                ...
                <div
                    class="mx-auto mt-4 h-[2px] w-24 rounded-full bg-gradient-to-r from-transparent via-white/40 to-transparent">
                </div>
            </div>
            ...
            <section class="space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <p class="text-sm font-semibold text-white/90">II. Pendapat Responden Tentang Pelayanan</p>
                        <p class="text-xs text-white/60">Pilih salah satu jawaban di setiap pertanyaan.</p>
                    </div>
                    <span class="inline-flex items-center rounded-full bg-white/10 border border-white/15 px-3 py-1 text-xs font-semibold text-white/80">
                        Skala 1–4
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    @foreach ($questions as $name => $q)
                        <div class="rounded-2xl border border-white/15 bg-white/10 p-4 sm:p-5">
                            <div class="flex items-start justify-between gap-3">
                                <p class="text-sm font-semibold text-white/90 leading-snug">
                                    {{ $q['title'] }}
                                </p>
                                <span class="shrink-0 inline-flex items-center rounded-lg bg-white/10 border border-white/15 px-2.5 py-1 text-[11px] font-semibold text-white/75">
                                    {{ strtoupper($name) }}
                                </span>
                            </div>
                            ...
                            <div class="mt-3 space-y-2">
                                @foreach ($q['options'] as $val => $label)
                                    <label
                                        class="group flex items-center justify-between gap-3 rounded-xl border border-white/15 bg-white/5 px-4 py-3
                                        ...
                                        <span class="flex items-center gap-3">
                                            ...
                                            >
                                            <span class="text-sm text-white/85">{{ $label }}</span>
                                        </span>

                                        <span class="text-xs font-semibold text-white/60 group-hover:text-white/80 transition">
                                            {{ $val }}
                                        </span>
                                    </label>
                                @endforeach
            ...
            <div class="space-y-1">
                ...
                <textarea id="comment" name="comment" rows="4"
                    class="mt-1 w-full rounded-xl border border-white/25 bg-white/15 backdrop-blur
            ...
            <button type="submit"
                class="w-full inline-flex items-center justify-center gap-2 rounded-xl
                       bg-white/25 px-5 py-3 text-base font-semibold text-white
            ...
            <div class="pt-1 text-center">
                <a href="{{ url('/') }}" class="text-sm font-medium text-white/70 hover:text-white underline">
                    Lewati survey
                </a>
            </div>
        </form>
