...
@php
    ...
    $badge = function (string $tone = 'neutral') {
        return match ($tone) {
            'emerald' => 'bg-emerald-400/10 text-emerald-100 border border-emerald-300/15',
            'rose'    => 'bg-rose-400/10 text-rose-100 border border-rose-300/15',
            default   => 'bg-white/7 text-white/85 border border-white/12',
        };
    };

    $card = 'rounded-[22px] border border-emerald-300/12 bg-white/8 backdrop-blur-xl';
@endphp

<div class="max-w-6xl mx-auto space-y-6">

    {{-- HERO (same as profil) --}}
    <section class="relative overflow-hidden rounded-[28px] border border-emerald-300/15 bg-[#111827]/55 backdrop-blur-xl
            ...
            <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative">
                        ...
                        <div class="relative h-14 w-14 rounded-3xl border border-white/10 bg-white/7 backdrop-blur
            ...
            <div class="space-y-4 sm:hidden">
                @forelse ($attendances as $a)
                    <div class="{{ $card }} p-5 space-y-4">
                        <div>
                            <p class="text-[11px] uppercase tracking-wider text-white/55">Tanggal</p>
                        ...
                        <div>
                            <p class="text-[11px] uppercase tracking-wider text-white/55">Lokasi</p>
                            <p class="text-sm font-semibold text-white">
                                {{ $a->location?->name ?? '-' }}
                            </p>
                        </div>
                ...
                @empty
                    <div class="{{ $card }} p-6 text-center">
                        ...
                        <p class="mt-1 text-xs text-white/70">
                            Data presensi akan muncul setelah check-in.
                        </p>
                    </div>
                @endforelse
            </div>
