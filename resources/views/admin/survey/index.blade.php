            ...
            <div class="relative">
                ...
                <div id="menuExportSurvey"
                    ...
                    <button type="button"
                        ...
                        data-url="{{ route('admin.survey.export.excel', request()->query()) }}"
                        data-label="Excel">
                    ...
                    <button type="button"
                        class="export-survey-action w-full text-left px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 flex items-center justify-between"
                        data-url="{{ route('admin.survey.export.pdf', request()->query()) }}"
                        data-label="PDF">
                ...
                @forelse($surveys as $survey)
                    @php
                        ...
                        foreach (array_keys($questions) as $k) {
                            $v = data_get($survey, $k);
                            if ($v !== null && $v !== '') $vals[] = (int) $v;
                        }
                        $avg = count($vals) ? (array_sum($vals) / count($vals)) : 0; // 1..4
                        $avgLabel = $avg ? number_format($avg, 2, ',', '.') : '-';

                        $avgBadge = $avg >= 3.50 ? 'bg-emerald-100 text-emerald-800' : ($avg >= 2.75 ? 'bg-amber-100 text-amber-800' : 'bg-rose-100 text-rose-800');
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $avgBadge }}">
                                ...
                                <div class="p-4 space-y-2">
                                    @foreach ($questions as $key => $qmeta)
                                        @php
                                            ...
                                            $pill = $v >= 4 ? 'bg-emerald-50 text-emerald-700' : ($v === 3 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700');
                                        @endphp

                                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 rounded-xl border border-slate-200 px-3 py-3">
                                            <div class="min-w-0">
                                                <div class="text-xs font-semibold text-slate-500">{{ strtoupper($key) }}</div>
                                            ...
                                            <div class="shrink-0">
                                                <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $pill }}">
                                                    Skor: {{ $v ?: '-' }}
                                                </span>
                                            </div>
                                        </div>
