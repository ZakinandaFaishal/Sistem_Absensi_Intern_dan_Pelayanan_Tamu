...
<div x-data="{ sidebarOpen: false }" class="min-h-screen w-full flex overflow-x-hidden">
    ...
    <aside
        ...
        class="fixed sm:static z-40 inset-y-0 left-0 w-72 sm:w-64
        ...
        @php
            ...
            $isActive = function(string $pathStartsWith) {
                return request()->is(ltrim($pathStartsWith, '/').'*');
            };
        @endphp

        {{-- Menu (scrollable kalau layar pendek) --}}
        <nav class="px-4 py-5 space-y-2 text-sm overflow-y-auto grow">
            <a href="{{ $urlProfile }}"
               class="flex items-center gap-3 rounded-xl px-3 py-2 transition
                      {{ $isActive('intern/userProfile') ? 'bg-white/25 font-semibold' : 'text-white/70 hover:bg-white/15 hover:text-white' }}">
            ...
            <a href="{{ $urlScanQr }}"
               class="flex items-center gap-3 rounded-xl px-3 py-2 transition
                      {{ $isActive('attendance') ? 'bg-white/25 font-semibold' : 'text-white/70 hover:bg-white/15 hover:text-white' }}">
                <span class="text-base">📷</span>
            ...
            <a href="{{ $urlHistory }}"
               class="flex items-center gap-3 rounded-xl px-3 py-2 transition
                      {{ $isActive('intern/attendance') ? 'bg-white/25 font-semibold' : 'text-white/70 hover:bg-white/15 hover:text-white' }}">
                <span class="text-base">📌</span>
