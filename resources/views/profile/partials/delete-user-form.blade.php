...
<section class="relative overflow-hidden rounded-[22px] border border-white/10 bg-[#111827]/55 backdrop-blur-xl shadow-[0_18px_55px_rgba(0,0,0,.45)] p-5 sm:p-6">
    ...
    {{-- top accent (single tone like profile) --}}
    <div class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-teal-300/45 via-white/10 to-transparent"></div>

    {{-- HEADER --}}
    <div class="flex items-center gap-3">
        <div class="h-11 w-11 rounded-2xl bg-rose-500/12 border border-rose-300/15 flex items-center justify-center">
    ...
    <div class="mt-5 space-y-4">
        <div class="rounded-2xl border border-rose-300/15 bg-rose-500/10 p-4">
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <div class="relative">
            {{-- top accent (danger only, subtle) --}}
            <div class="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-rose-400/55 via-white/10 to-transparent"></div>
            ...
            <form method="post" action="{{ route('profile.destroy') }}" class="p-5 sm:p-6 space-y-5">
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3">
                        <span class="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/12 bg-white/10">
                            <x-icon name="trash" class="h-5 w-5 text-rose-200" />
                        </span>
