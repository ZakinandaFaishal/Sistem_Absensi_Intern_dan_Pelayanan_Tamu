...
<style>
    ...
    .bar-anim { transform-origin: bottom; animation: barGrow .75s cubic-bezier(.2,.95,.2,1) both; }

    .soft-ring:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(15,23,42,.12); }
    ...
    .card-hover:hover .bubble{ transform: scale(1.08); }
    ...
    .btn-shine:hover::after{ transform: translateX(220%); }
...
<div class="anim grid grid-cols-1 xl:grid-cols-3 gap-4">
    ...
    {{-- CHART --}}
    <div class="fade-up xl:col-span-2 relative overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm p-5 card-hover">
        <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-slate-50 via-transparent to-transparent"></div>
        ...
        <div class="relative flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 transition hover:bg-slate-200">
                    ...
                    @foreach($chart as $i => $point)
                        ...
                        <div class="group tip-wrap relative flex flex-col items-center justify-end gap-2">
                            ...
                            <div class="w-full flex items-end justify-center gap-1">
                                <div
                                    class="bar-anim w-2 rounded-md bg-emerald-600/75 shadow-sm
                                ...
                                <div
                                    class="bar-anim w-2 rounded-md bg-sky-600/75 shadow-sm
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-4">
        ...
        <div class="fade-up relative overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm p-5 card-hover">
            <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-emerald-50 via-transparent to-transparent"></div>
            <div class="relative flex items-start justify-between">
                <div>
                    <p class="text-xs text-slate-500">Buku Tamu Hari Ini</p>
                    <p class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ $fmt($stats['guest_today'] ?? 0) }}</p>
                    <p class="mt-1 text-xs text-slate-500">Total kunjungan hari ini</p>
                </div>
                <div class="bubble h-11 w-11 rounded-xl bg-emerald-50 text-emerald-700 flex items-center justify-center">
                    📝
                </div>
            </div>
        </div>
        ...
        <div class="fade-up relative overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm p-5 card-hover">
            <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-sky-50 via-transparent to-transparent"></div>
            <div class="relative flex items-start justify-between">
                <div>
                    <p class="text-xs text-slate-500">Survey Masuk</p>
                    <p class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ $fmt($stats['survey_today'] ?? 0) }}</p>
        ...
        <div class="fade-up relative overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm p-5 card-hover">
            <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-fuchsia-50 via-transparent to-transparent"></div>
            <div class="relative flex items-start justify-between">
                <div>
                    <p class="text-xs text-slate-500">Total Users</p>
                    <p class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ $fmt($stats['users_total'] ?? 0) }}</p>
                    <p class="mt-1 text-xs text-slate-500">Akun aktif terdaftar</p>
                </div>
                <div class="bubble h-11 w-11 rounded-xl bg-fuchsia-50 text-fuchsia-700 flex items-center justify-center">
                    👥
                </div>
            </div>
        </div>
