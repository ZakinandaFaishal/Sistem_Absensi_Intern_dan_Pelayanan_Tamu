...
@php
    $backUrl =
        \Illuminate\Support\Facades\Route::has('attendance.qr')
            ? route('attendance.qr')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <section class="relative rounded-3xl border border-white/15 bg-white/10 backdrop-blur-xl shadow-xl overflow-hidden">
        ...
        <form method="POST" action="{{ route('attendance.scan.store') }}" id="attendance-scan-form" class="relative p-6 sm:p-7 space-y-4">
            @csrf
            ...
            <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs sm:text-sm text-white/70 leading-relaxed">
                Tips: Pastikan GPS aktif dan izin lokasi untuk browser diizinkan. Jika lokasi belum tersedia, tunggu beberapa detik lalu coba lagi.
            </div>
...
(function() {
    ...
    function setStatus(text, variant = 'idle') {
        ...
        statusEl.innerHTML = `<span id="geo-dot" class="${dotEl ? dotEl.className : 'inline-block h-2 w-2 rounded-full bg-white/40'}"></span>${text}`;
    }
